...
public sealed class DevicesController(AppDbContext db, AirPageClient client) : ControllerBase
{
    [HttpGet] public async Task<IActionResult> List(CancellationToken ct) =>
        Ok(await db.Devices.Select(x => new { x.Id, x.Name, x.Origin, x.Width, x.Height, x.Mode, x.IsDefault, x.CreatedAt }).ToListAsync(ct));
    [HttpPost] public async Task<IActionResult> Add(AddDeviceRequest request, CancellationToken ct)
    {
        var (device, _) = client.ParseAndProtect(request.Name, request.DeviceUrl, request.IsDefault);
        if (request.IsDefault) await db.Devices.ExecuteUpdateAsync(x => x.SetProperty(d => d.IsDefault, false), ct);
        if (!await db.Devices.AnyAsync(ct)) device.IsDefault = true;
    ...
    {
        if (!await db.Devices.AnyAsync(x => x.Id == id, ct)) return NotFound();
        await db.Devices.ExecuteUpdateAsync(x => x.SetProperty(d => d.IsDefault, d => d.Id == id), ct);
        return NoContent();
    }
    [HttpDelete("{id:guid}")] public async Task<IActionResult> Delete(Guid id, CancellationToken ct)
    {
        await db.Devices.Where(x => x.Id == id).ExecuteDeleteAsync(ct); return NoContent();
    }
}
