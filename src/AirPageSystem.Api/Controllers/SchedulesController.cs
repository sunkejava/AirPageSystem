...
public sealed class SchedulesController(AppDbContext db, PanelExecutionService executor,CurrentUser current):ControllerBase
{
 [HttpGet] public async Task<IActionResult> List(CancellationToken ct)=>Ok(await db.Schedules.Where(x=>x.OwnerUserId==current.Id).ToListAsync(ct));
 [HttpPost] public async Task<IActionResult> Save(SaveScheduleRequest r,CancellationToken ct)
 {
  ...
  await current.DemandAsync("schedules.manage",ct);var x=new ScheduleDefinition{Name=r.Name,TemplateId=r.TemplateId,DeviceId=r.DeviceId,RetryPolicyId=r.RetryPolicyId,Cron=r.Cron,TimeZoneId=r.TimeZoneId,Enabled=r.Enabled,NextRunAt=ScheduleTime.Next(r.Cron,r.TimeZoneId,DateTimeOffset.UtcNow),OwnerUserId=current.Id};
 ...
 {
  await current.DemandAsync("schedules.manage",ct);var x=await db.Schedules.FirstOrDefaultAsync(x=>x.Id==id&&x.OwnerUserId==current.Id,ct);if(x is null)return NotFound();
  var error=await ValidateReferences(r,ct);if(error is not null)return error;
  x.Name=r.Name;x.TemplateId=r.TemplateId;x.DeviceId=r.DeviceId;x.RetryPolicyId=r.RetryPolicyId;x.Cron=r.Cron;x.TimeZoneId=r.TimeZoneId;x.Enabled=r.Enabled;
 ...
 {
  await current.DemandAsync("schedules.manage",ct);var x=await db.Schedules.FirstOrDefaultAsync(x=>x.Id==id&&x.OwnerUserId==current.Id,ct);if(x is null)return NotFound();x.Enabled=!x.Enabled;
 ...
 [HttpPost("{id:guid}/run")] public async Task<IActionResult> Run(Guid id,CancellationToken ct)
 {
  ...
  try
  {
   var result=await executor.ExecuteAsync(x.TemplateId,x.DeviceId,true,ct,current.Id,x.RetryPolicyId);
 ...
 [HttpDelete("{id:guid}")] public async Task<IActionResult> Delete(Guid id,CancellationToken ct){await current.DemandAsync("schedules.manage",ct);await db.Schedules.Where(x=>x.Id==id&&x.OwnerUserId==current.Id).ExecuteDeleteAsync(ct);return NoContent();}
